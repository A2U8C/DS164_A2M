               ...
               /* if(dataSnapshot.exists() && dataSnapshot.child("lat").getValue()!=null && dataSnapshot.child("lon").getValue()!=null
                ...
                {*/
                    for(DataSnapshot ds : dataSnapshot.getChildren()){
                        ...
                        String tp_lat = ds.child("geo_lat").getValue().toString();
                        String tp_long = ds.child("geo_long").getValue().toString();

                        //Double lat = ds.child("geo_lat").getValue(double.class);
                      //  Double lon = ds.child("geo_long").getValue(double.class);
                        Double lat = Double.parseDouble(tp_lat);
                        Double lon = Double.parseDouble(tp_long);
                        String tt_type = ds.child("type").getValue().toString();
                        String tt_title = ds.child("time_date_case").getValue().toString();
                        if(tt_type.equals("Smuggling"))
                        {
                        ...
                        else if(tt_type.equals("Poaching"))
                        {
                            LatLng locaa = new LatLng(lat,lon);
                            // mMap.addMarker(new MarkerOptions().position(locaa).title("IMP"));
                            // new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.tree_cut));
